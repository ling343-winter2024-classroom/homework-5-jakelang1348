...
(TODO insert introduction to the data)
...
*"Data Analysis*. Item 29 was removed due to a coding error. Regression models for dependent variables (Error Rates, Response Times) were constructed using either the categorical factor (Expectation, sum coded 0.5 \[unexpected\] and –0.5 \[expected\]) or graded by-item noun cloze probabilities (Noun Cloze, centered prior to model fit). Error rates were analyzed using Firth’s penalized likelihood method ([Firth, 1993](https://escholarship.org/uc/item/7dz7z3q3#B6)), using *logistf* ([Heinze, Ploner, & Jiricka, 2020](https://escholarship.org/uc/item/7dz7z3q3#B10)). This analysis was considered more appropriate than mixed-effects logistic regression given the very low error rates.

Words with error responses (including post-error ‘correct’ responses) were removed and the remaining response times for all correct-on-first-attempt words were analyzed by fitting mixed-effects models with maximal random effects using *lme4* ([Barr, et al., 2013](https://escholarship.org/uc/item/7dz7z3q3#B2)) and *p*-values determined by *lmerTest* ([Kuznetsova, Brockhoff, & Christensen, 2017](https://escholarship.org/uc/item/7dz7z3q3#B14)) using the Satterthwaite approximation. Conditional means were derived from the models using *emmeans* ([Lenth, 2021](https://escholarship.org/uc/item/7dz7z3q3#B15)) and difference-adjusted 95% (percentile) mixed-effect-model-based intervals were calculated to account for crossed within-subjects and within-items random effects ([Politzer-Ahles, 2017](https://escholarship.org/uc/item/7dz7z3q3#B23)). Analyses are reported using untransformed RT, though log(RT) revealed similar patterns. Raw data and analysis scripts are available at <https://osf.io/frdtm/>. Additional analyses are reported in the appendix."
...
Let's print a table showing the mean, min, max, and standard deviation of participant ages.
...
This figure shows the response times, having removed error and post-error responses. We look to see the response times for the unexpected condition and the expected condition for boht the nouns and the preceding article.
